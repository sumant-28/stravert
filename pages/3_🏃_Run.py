...
def calculate_km_splits_from_cumulative2(cum_time, cum_dist):
    ...
    for i in range(1, len(cum_dist)):
        if cum_dist[i] >= target_km:
            ...
            split = split_time - last_time
            if type(target_km) != int:
                split = split / (target_km % 1)
            splits.append(split)  # time for this km split
...
sc = explosion('stepCount', d3)
sc = fix_all_timestamps_in_df(sc)
ae = explosion('activeEnergy', d3)
ae = fix_all_timestamps_in_df(ae)
hrd = explosion('heartRateData', d3)
...
formatted = d3['start'].loc[0].strftime("%B %d, %Y at %-I:%M %p")
formatted = formatted[:-2] + formatted[-2:].lower()
st.subheader(f"{formatted}")
st.subheader(f"{round(d3['temperature'].loc[0]['qty'])} °C, {round(d3['humidity'].loc[0]['qty'])}% Humidity")
...
with col1:
    st.metric(label="Distance", value=f"{round(d3['distance'][0]['qty'],2)} km")
    st.metric(label="Time", value=t3)
    st.metric(label="Calories", value=f"{round(d3['activeEnergyBurned'][0]['qty'])} Cal")


with col2:
    st.metric(label="Avg Pace", value=f"{minutes}:{seconds:02d}")
    st.metric(label="Elevation Gain", value=f"{int(d3['elevationUp'][0]['qty'])} m")
    st.metric(label="Avg Heart Rate", value=f"{round(sum(hrd['Avg'])/len(hrd))} bpm")

st.header("Pace Analysis")

cdate = list(cd['date'])
cdate.append(d3['end'].loc[0])
...
if math.floor(cumulative_distance_km[len(cumulative_distance_km) - 1]) != math.floor(cumulative_distance_km[len(cumulative_distance_km) - 2]):
    x_end = x[-1]
    ax.bar(x[-1], values[-1], width=full_width, align='edge',
       color='lightblue', edgecolor='black')
...
plt.fill_between(distance_km[1:len(distance_km)], pace_min_per_km, 10, color='blue', alpha=0.4)

# Show in Streamlit
st.pyplot(fig)

st.subheader(f"Avg Pace {minsec_formatter((sum(pace_min_per_km)/len(pace_min_per_km)),0)} /km")
...
averages = sc['qty']
#averages = averages[::-1]
d1 = d3['end'].loc[0] - d3['start'].loc[0]
...
# Time-weighted sum
weighted_sum = np.sum(averages[:n_full])  # full minutes
if fractional > 0:
    weighted_sum += averages[n_full] * fractional  # add fractional weight
